package com.excel.process.corner;

import com.excel.process.cfg.CellFmt;
import com.excel.process.other.DateUtil;
import com.excel.process.other.ExcelDataReflectUtil;
...
public class PoiCellProcess {
	private static final Logger LOGGER = LoggerFactory.getLogger(PoiCellProcess.class);
	...
	public static void writeCellValue(SXSSFCell bodyCell, Object value, CellFmt cellFmt) {
		if(null == cellFmt){
			writeCellValueDefault(bodyCell, value);
		}else{
	...
	public static void writeCellValueByFmt(SXSSFCell bodyCell, Object value, String cellFmt){
			...
			if(value.getClass().getSimpleName().equals("LocalDate")){
				bodyCell.setCellValue(DateUtil.format((LocalDate)value,DateUtil.FORMAT_YYYY_MM_DD));
				return;
			}
			if(value.getClass().getSimpleName().equals("LocalDateTime")){
                bodyCell.setCellValue(Date.from(((LocalDateTime)value).atZone(DateUtil.CHINA_ZONE_ID).toInstant()));
				return;
			}
			/**
	...
	public static void writeCellValueDefault(SXSSFCell bodyCell, Object value) {
		...
		if (value.getClass().getSimpleName().equals("Timestamp")) {
			...
			bodyCell.setCellValue(DateUtil.format(((Timestamp) value).toLocalDateTime(), DateUtil.FORMAT_YYYY_MM_DD_HMS));
			return;
		}
		...
		if (value.getClass().getSimpleName().equals("Date")) {
			bodyCell.setCellValue(DateUtil.format((Date) value, DateUtil.FORMAT_YYYY_MM_DD_HMS));
			return;
		}
		if (value.getClass().getSimpleName().equals("Character")) {
		...
		if(value.getClass().getSimpleName().equals("LocalDateTime")){
			bodyCell.setCellValue(DateUtil.format((LocalDateTime)value,DateUtil.FORMAT_YYYY_MM_DD_HMS));
			return;
		}
		/**
		 * 文本添加删除线 RichTextString 单元格批注 Comment 单元格错误值 ErrorValue(byte) 文本公式设置
