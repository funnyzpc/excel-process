...
import com.excel.process.corner.PoiCellProcess;
import com.excel.process.corner.PoiCellStyleProcess;
 ...
 * @author: funnyzpc
 **/
public class PoiExcelWriteUtil {
    private static final Logger LOGGER = LoggerFactory.getLogger(PoiExcelWriteUtil.class);
    ...
    public static <T extends Object> File toXlsxByObj(
        ...
        if(null != headerNames && headerNames.length != cellNames.length){
            LOGGER.error(HEADER_NAME);
            return null;
        }
        if(null != cellFmts && cellFmts.length != cellNames.length){
            LOGGER.error(CELL_LENGTH_ERROR_MSG);
            return null;
        }
        /**
         * 构造文件夹
        ...
        fileObject = new File(fileObject.getPath().concat(File.separator).concat(UUIDUtil.mediumUUID()).concat(EXPORT_FILE_END));
        SXSSFSheet sheet ;
